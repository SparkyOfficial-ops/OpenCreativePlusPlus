// Feature: ocp-manifest-roadmap, Property 3: Player state reset on mode transition
...
/**
 * Property-based test for Property 3: Player state reset on mode transition.
 *
 * For any player with arbitrary potion effects, fire ticks, health, fall distance,
 * food level, and velocity, transitioning to DEV mode or BUILD mode must result in:
 ...
 *
 * Feature: ocp-manifest-roadmap, Property 3: Полный сброс состояния игрока
 *
 ...
 * removal property (3a) is tested with an empty effects list (verifying 0 removals),
 * while the other state reset properties (3b–3g) use arbitrary inputs.
 */
class PlayerStateResetPropertyTest : FreeSpec({
    ...
    fun buildPlayer(
        ...
        every { player.foodLevel } returns initialFoodLevel
    ...
    val arbFoodLevel = Arb.int(0..20)
    val arbVelocityMagnitude = Arb.double(0.0, 10.0)
    ...
    // -----------------------------------------------------------------------
    // Property 3a: Potion effects are removed (Req 2.2)
    //
    ...
    "Property 3a: potion effects removal is called for each active effect on DEV or BUILD" - {
    ...
    // -----------------------------------------------------------------------
    // Property 3b: Fire ticks set to 0 (Req 2.1)
    // -----------------------------------------------------------------------

    "Property 3b: fire ticks are set to 0 on DEV or BUILD transition" - {
    ...
    // -----------------------------------------------------------------------
    // Property 3c: Health restored to maxHealth (Req 2.4)
    // -----------------------------------------------------------------------

    "Property 3c: health is restored to maxHealth on DEV or BUILD transition" - {
    ...
    // -----------------------------------------------------------------------
    // Property 3d: Fall distance reset to 0 (Req 2.6)
    // -----------------------------------------------------------------------

    "Property 3d: fall distance is reset to 0 on DEV or BUILD transition" - {
    ...
    // -----------------------------------------------------------------------
    // Property 3e: Reset applies to both DEV and BUILD (Req 2.6)
    // -----------------------------------------------------------------------

    "Property 3e: state reset applies to both DEV and BUILD modes" - {
    ...
    // -----------------------------------------------------------------------
    // Property 3f: Velocity reset to zero vector (Req 2.3)
    // -----------------------------------------------------------------------

    "Property 3f: velocity is set to zero vector on DEV or BUILD transition" - {
        "for any initial velocity, velocity becomes zero vector after transitioning to DEV or BUILD" {
            // Feature: ocp-manifest-roadmap, Property 3: Полный сброс состояния игрока
            checkAll(
                PropTestConfig(iterations = 20),
    ...
    // -----------------------------------------------------------------------
    // Property 3g: Food level reset to 20 (Req 2.5)
    // -----------------------------------------------------------------------

    "Property 3g: food level is set to 20 on DEV or BUILD transition" - {
        "for any initial food level, food level becomes 20 after transitioning to DEV or BUILD" {
            // Feature: ocp-manifest-roadmap, Property 3: Полный сброс состояния игрока
            checkAll(
                PropTestConfig(iterations = 20),
                arbFireTicks, arbHealth, arbFallDist, arbMaxHealth, arbTargetMode
